...
test("updating a record with a 422 error marks the records as invalid", function(){
  ...
  var errors = { name: ["can't be blank"], updated_at: ["can't be blank"]};
  ...
  expectState('valid', false);
  deepEqual(person.get('errors'), Ember.Object.create({ name: ["can't be blank"], updatedAt: ["can't be blank"] }), "the person has the errors");
});
